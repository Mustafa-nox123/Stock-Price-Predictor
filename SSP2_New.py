...
def test_agent_and_save_predictions(agent, test_data, output_file='test_predictions.csv'):
    ...
    return results_df
# new addition of pso code
import time
def pso_fitness(params, agent, train_data):
    """
    The fitness function that evaluates the performance of an agent based on the hyperparameters
    passed from the Particle Swarm Optimization (PSO) algorithm.
    """
    # Unpack hyperparameters
    learning_rate, gamma, epsilon, batch_size, dropout = params
    
    print(f"Evaluating params: {params}")  # Debug print to track params

    # Modify the agent's hyperparameters (no re-initialization, just setting new values)
    agent.learning_rate = learning_rate
    agent.gamma = gamma
    agent.epsilon = epsilon
    agent.batch_size = int(batch_size)
    agent.dropout = dropout

    # Train the agent (for a few episodes for speed)
    total_reward = 0
    env = StockTradingEnv(train_data, verbose=False)  # Initialize the environment

    max_episodes = 1  # Limit episodes to speed up evaluation
    for episode in range(max_episodes):
        state = env.reset()
        done = False
        while not done:
            # Limit actions per training to avoid time limit issues
            action = agent.act(state)
            next_state, reward, done, _ = env.step(action)
            agent.remember(state, action, reward, next_state, done)
            state = next_state
    ...
    print(f"Total reward for params {params}: {total_reward}")  # Debug print to track reward
    return -total_reward  # Negative since PSO minimizes
...
def main():
    ...
    try:
        ...
        df = df.ffill().bfill()  # Updated to use ffill() and bfill()
        df = df.dropna()
    ...
    state_size = train_data.shape[1]  # Number of features (columns)
    action_size = 3  # Number of actions: Buy, Sell, Hold

    # Initialize agent only once before running PSO optimization
    agent = DQNAgent(state_size, action_size)  # Only create once

    # PSO hyperparameter optimization
    lb = [0.0001, 0.90, 0.01, 16, 0.0]  # Lower bounds
    ub = [0.01, 0.99, 1.00, 64, 0.5]   # Upper bounds

    # Pass train_data, state_size, and action_size to the fitness function
    best_params, _ = pso(lambda params: pso_fitness(params, agent, train_data), lb, ub, swarmsize=5, maxiter=3)
    learning_rate, gamma, epsilon, batch_size, dropout = best_params
    print(f"Best params from PSO: {best_params}")

    # Now, use these best params to update and train your real agent
    agent.learning_rate = learning_rate
    agent.gamma = gamma
    agent.epsilon = epsilon
    agent.batch_size = int(batch_size)
